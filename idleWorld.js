...
function renderSpecial() { //renders the area-unique things, like the enemy, the trees, etc.
	if (!loginScreen) {
		if (inMainScreen) {
			...
			var timeLeftRatio = (charStats.killTime - ((Date.now() - dateStart)/1000)).toPrecision(3) / charStats.killTime;
			if (timeLeftRatio <= 0) {
				isMonsterDead = true;
				setTimeout(function() {
					dateStart = Date.now();
			...
			if (!isMonsterDead) {
				...
				ctx.textAlign = "center";
				ctx.textBaseline = "middle";
				ctx.fillText((charStats.killTime - ((Date.now() - dateStart)/1000)).toPrecision(3) + " S", 400, 440);
			};
		};
	};
}
